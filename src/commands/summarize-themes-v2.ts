  ...
  await runPool(
    ...
    async (theme, index, total) => {
      ...
      try {
        ...
        try {
          finalSections = await ai.generateContent<any>(
            structurePrompt,
          ...
          console.warn(`   ⚠️  JSON structuring failed; saving narrative summary instead: ${structureErr instanceof Error ? structureErr.message : String(structureErr)}`);
          finalSections = { executiveSummary: finalAnalysis };
        }
